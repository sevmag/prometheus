...
RES=os.path.join(PR,"resources"); OUT=os.path.join(RES,"PPC_tables","arca_water")
...
def rd(p):
    ...
    for ln in open(p):
        ln=ln.strip()
        if not ln or ln[0]=="#" or ln[0].isalpha(): continue
...
with open(os.path.join(OUT,"sca_len.dat"),"w") as f:
    for w,L in zip(sw,sl): f.write(f"{w:.4f} {L:.6f}\n")
with open(os.path.join(OUT,"abs_len.dat"),"w") as f:
    for w,L in zip(aw,al): f.write(f"{w:.4f} {L:.6f}\n")
...
with open(os.path.join(OUT,"icemodel.dat"),"w") as f:
    for d in range(500,4001,100): f.write(f"{d}.0 0.02 0.01 0.0\n")
...
with open(os.path.join(OUT,"cfg.txt"),"w") as f:
    f.write("# arca water: over-R, eff, HG(0), g, [aniso], [holeice], [aniso2], [absaniso]  -- BFR omitted => OFF\n")
    for v in ["5","1.0","0","0.92","130.0","0.0","0.0","0.0","0.03","100","0.0","0.92","0.0","0.0","0.0","0.0"]:
        f.write(v+"\n")
...
cdf/=cdf[-1]
g2=[grid[0]]; c2=[0.0]                        # keep only strictly-increasing CDF (drop QE~0 tail)
for i in range(1,len(grid)):
    if cdf[i] > c2[-1] + 1e-6: g2.append(grid[i]); c2.append(cdf[i])
c2[-1]=1.0                                    # force exact 1 at last kept point
with open(os.path.join(OUT,"wv.dat"),"w") as f:
    for c,wl in zip(c2,g2): f.write(f"{c:.8f} {wl:.0f}.\n")
# 5) om.dirs: 1000 uniform sphere directions (idx x y z)
with open(os.path.join(OUT,"om.dirs"),"w") as f:
    n=1000; ga=math.pi*(3-math.sqrt(5))
    for i in range(n):
        z=1-2*(i+0.5)/n; r=math.sqrt(max(0,1-z*z)); th=ga*i
...
with open(os.path.join(OUT,"om.wv_1.0"),"w") as f:
    for w,q in zip(qw,qq): f.write(f"{w:.1f} {(q/100.0)*AREA:.6f}\n")
# 7) eff-f2k, rnd.txt, as.dat from upstream (parse-compatible)
for fn in ("eff-f2k","rnd.txt","as.dat"): shutil.copy(os.path.join(UP,fn),os.path.join(OUT,fn))
print("wrote",OUT)
