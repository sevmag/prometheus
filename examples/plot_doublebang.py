...
df = pd.read_parquet(
    max(glob.glob(PR + "/output/arca_multipmt_tau/*_photons.parquet"), key=os.path.getmtime)
)
...
def inside(pt):
    return (ZMIN - 50 < pt[2] < ZMAX + 50) and abs(pt[0]) < RXY + 50 and abs(pt[1]) < RXY + 50


# ---- select the clearest contained double-bang ----
best_i, best_score, meta = None, -1e9, None
for i in range(len(df)):
    if not len(df["photons"].iloc[i]["t"]):
        continue
    mt = df["mc_truth"].iloc[i]
    v = np.array([mt["initial_state_x"], mt["initial_state_y"], mt["initial_state_z"]])
    F = np.stack(
        [
            np.atleast_1d(mt["final_state_x"]),
            np.atleast_1d(mt["final_state_y"]),
            np.atleast_1d(mt["final_state_z"]),
        ],
        1,
    )
    far = F[np.linalg.norm(F - v, axis=1) > 100]
    if len(far) == 0 or not inside(v):
        continue
    c2 = far.mean(0)
    sep = np.linalg.norm(c2 - v)
    x, y, z, t = hits(i)[:4]
    ph = np.stack([x, y, z], 1)
    n1 = int((np.linalg.norm(ph - v, axis=1) < 110).sum())
    n2 = int((np.linalg.norm(ph - c2, axis=1) < 110).sum())
    if not (n1 > 80 and n2 > 30 and 200 < sep < 520):
        continue
    score = -abs(sep - 330)
    if score > best_score:
        best_i, best_score, meta = i, score, (v, c2, sep, n1, n2, len(t))
if best_i is None:
    best_i = max(range(len(df)), key=lambda i: len(df["photons"].iloc[i]["t"]))
    mt = df["mc_truth"].iloc[best_i]
    v = np.array([mt["initial_state_x"], mt["initial_state_y"], mt["initial_state_z"]])
    F = np.stack(
        [
            np.atleast_1d(mt["final_state_x"]),
    ...
    c2 = F[int(np.argmax(np.linalg.norm(F - v, axis=1)))]
    meta = (v, c2, np.linalg.norm(c2 - v), 0, 0, len(df["photons"].iloc[best_i]["t"]))
...
np.add.at(py, inv, y)
np.add.at(pz, inv, z)
pte = np.full(nP, np.inf)
np.minimum.at(pte, inv, t)
px /= cnt
py /= cnt
pz /= cnt
...
sc = ax.scatter(
    ...
    py[order],
    pz[order],
    s=6 + 60 * np.log10(cnt + 1) / np.log10(cnt.max() + 1),
    c=pte[order],
    cmap="rainbow_r",
    alpha=0.9,
    edgecolors="none",
)
fig.colorbar(sc, ax=ax, pad=0.02, shrink=0.55).set_label(
    "earliest hit time [ns] (red=earlier, violet=later)"
)
...
ax.set_zlabel("z [m]")
ax.set_title(
    f"ARCA multi-PMT 10 PeV $\\nu_\\tau$ CC in water "
    f"(event {best_i}, {nhit} hits, {nP} PMTs)\n"
    "circle size $\\propto\\log_{10}$(hits/PMT); tau double-bang"
)
ax.legend(loc="upper left", fontsize=9)
ax.view_init(elev=12, azim=np.degrees(np.arctan2(c2[1] - v[1], c2[0] - v[0])) + 90)
...
for idx, (zen, az) in pdirs.items():
    ...
    if idx in hm:
        ...
        ax2.scatter(
            *P,
            s=40 + 160 * np.log10(cc + 1) / np.log10(pcnt.max() + 1),
            c=[tt],
            cmap="rainbow_r",
            vmin=b_t.min(),
            vmax=b_t.max(),
...
ax2.set_title(
    f"multi-PMT view: DOM (string {b_st}, om {b_om}) — {len(ups)}/31 PMTs hit\n"
    "gray=no hit; size $\\propto\\log_{10}$(hits); color=time; arrows toward the two cascades"
)
ax2.set_xlabel("x")
ax2.set_ylabel("y")
ax2.set_zlabel("z (up)")
for s in (ax2.set_xlim, ax2.set_ylim, ax2.set_zlim):
    s(-1.3, 1.3)
fig.tight_layout()
