...
If you prefer a pre-configured environment (recommended for macOS, Windows, or clusters), see [Container Installation](containers.md).

## Before You Start

Prometheus relies on several scientific software components that can be difficult to build manually. To simplify this, we provide an automated installer that sets up everything for you.
...
!!! note
    Native installation is only fully supported on Linux. On macOS and Windows, use containers instead.
...
!!! warning
    The environment must be active whenever you run Prometheus. If it is not activated, commands like `python examples/...` may fail or use the wrong dependencies.
...
- **Build failures (PROPOSAL / LeptonInjector)**: Use containers instead.
- **Missing optional Python packages**: Run `pip install uproot` or `pip install pandas`.
